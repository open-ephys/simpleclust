...
%tbins=[min(mua.ts):0.5/1000:max(mua.ts)]; % make .5ms bins (this will blow up for big files on small machines)

% instead do coarse histogram, use it as lookup table to allow fast spike
% by spike comparisons
lookup_binsize = 50; % in ms, smaller -> more ram use, larger -> more cpu time
        ...
        if sum(h_others(spikebins(i)))>0 % any?
            for j=1:numel(mua.otherchannels) % for all others
                if h_others(spikebins(i),j)>0
                    ...
                    if min(abs( mua.ts(i)- mua.otherchannels{j}.ts(otherid) )) < .5/1000; % chance timw window here, or even add penalty based on D_t?
